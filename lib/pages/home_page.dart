  ...
  groupList() {
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        // make some checks
        if (snapshot.hasData) {
          final group = snapshot.data!.data() as Map;
          if (group['groups'] != null) {
            if (group['groups'].length != 0) {
              return ListView.builder(
                itemCount: group['groups'].length,
                itemBuilder: (context, index) {
                  int reverseIndex = group['groups'].length - index - 1;
                  return GroupTile(
                      groupId: getId(group['groups'][reverseIndex]),
                      groupName: getName(group['groups'][reverseIndex]),
